    ...
    reach.reachableBlock = ^(Reachability * reachability)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if( _list != (id)[NSNull null] && [_list count] == 0) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(queryMarkFinished1:)
                                                             name:@"BLQueryMarkFinishedNotification1" object:nil];
                
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(queryMarkFailed1:)
                                                             name:@"BLQueryMarkFailedNotification1" object:nil];
                _queryKey = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                             @"1",@"currentPage",
                             nil];
                [[Markjson sharedManager] selectKey1:_queryKey];
            }
        });
    };
...
//接收BL查询Hotel信息成功通知
- (void)queryMarkFinished1:(NSNotification*)not
{
    self.tableview.tableFooterView = nil;
    self.tableview.pullTableIsRefreshing = NO;
    self.tableview.pullTableIsLoadingMore = NO;
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"BLQueryMarkFinishedNotification1"object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"BLQueryMarkFailedNotification1" object:nil];
...
//接收BL查询Hotel信息失敗通知
- (void)queryMarkFailed1:(NSNotification*)not
{
    self.tableview.tableFooterView = nil;
    self.tableview.pullTableIsRefreshing = NO;
    self.tableview.pullTableIsLoadingMore = NO;
